...
## --> eror in plot - lot of NAs at the top
...
# PM2.5 Results with new data & models

## SP 2019 PM2.5 results from current models
...
pm2.5 <- hrs %>%
  filter(grepl("Main", description),
        grepl("PM2.5", pollutant),
        exposure_duration == 10,
        # dont include SP or ST NO2 sensitivity analysis
        !exposure_model %in% c("2019 ST", 
                               "ST + 2019 ST", 
                               "ST")
        ) %>%  
  mutate(
...
pm2.5 %>%
  ...
  labs(x= "HR (95% CI)", y="SP PM2.5 Model",
       linetype="Analysis", col="Analysis", shape="Analysis"
       )
  # ggplot(., aes(y=model, x=hr, xmin=lower95, xmax=upper95,)) +
  # facet_grid(rows = vars(pollutant_predictors),
  #            cols = vars(exposure_model),
  #            switch = "both") +
  # geom_vline(xintercept = 1, alpha=0.5, linetype=2) + 
  # geom_pointrange(position = position_dodge(width = 1), aes(linetype=description,
  ...
  #                                                           shape=description
  #                                                           )) +
  # geom_text(aes(label=hr_label, x=.7, hjust=.2, fontface="bold"))+
  # labs(x= "HR (95% CI)", y="SP 2019 PM2.5 Exposure",
  #      linetype="Analysis", col="Analysis", shape="Analysis"
  #      )

ggsave(file.path(image_path, "Other", "PM2.5", "hrs_sp_pm2.5.png"), width = 7, height = 6)

```

## ST PM2.5 results using the same dataset 

* these have the spatial coverage requirement like main analysis w/ MM
...
ggsave(file.path(image_path, "Other", "PM2.5", "hrs_st_pm2.5_v2.png"), width = 10, height = 7)
