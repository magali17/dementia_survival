...
```{r}
hrs %>%
  filter(grepl("single pollutant", model, ignore.case = T),
         grepl("PM2.5", pollutant, ignore.case = T)
         ) %>%
  plot_hrs2() + 
  facet_grid(rows = vars(exposure_model), switch = "both")
...
```{r}
# these factor levels are wrong
# new_pollutant_levels <- gsub("PNC (1000 pt/cm3)\n", "", pollutant_levels, fixed = T) %>%
#    gsub("(pt/cm3)", "", ., fixed = T) %>%
#   gsub("\nOnroad", "P-TRAK Onroad 20-1,0000 nm", .)
ufp_pollutant_levels <- c("NanoScan (10-420 nm), Primary", "P-TRAK (20-1,0000 nm)", "DiSCmini (10-700 nm)", 
...
ufp_hrs <- hrs %>%
  filter(#grepl("Main", description),
          grepl("Main|PNC|Particle", description),
  ...
  gsub("Onroad", "P-TRAK Onroad 20-1,0000 nm", .),
        ...
        #pollutant = factor(pollutant, levels =new_pollutant_levels)
...
ggplot(ufp_hrs, aes(y=pollutant, col=model, x=hr, xmin=lower95, xmax=upper95,)) + 
    ...
    geom_pointrange(position = position_dodge(width = 1)) +
    geom_text(aes(label=hr_label, x=Inf, hjust=1), position = position_dodge(width = 1), size=3)+
    labs(x= "HR", y="UFP Model", col = "Exposure Model", shape="")
...
different no2 models 

```{r}
no2_sensitivity <- hrs %>%
  filter( 
    grepl("NO2", pollutant),
    pollutant_predictors == "NO2 + PM2.5",
    grepl("Main", description)) %>% 
  mutate(exposure_model = gsub(" \\+ 2019 ST", "", exposure_model),
         exposure_model = gsub("MM", "Mobile Monitoring (Main Analysis)", exposure_model),
         exposure_model = gsub("ST", "Spatiotemporal", exposure_model)) 

no2_sensitivity %>%  
  select(exposure_model, hr_label) %>%
  kable(caption = "NO2 sensitivity analyses of the estimated hazard ratios (95% CI) for the incidence of all-cause dementia associated with an increase in ten-year average pollutant exposure level, as determined by different NO2 exposure models.", 
        col.names = c("NO2 Exposure Model", "HR (95% CI)")
        ) %>%
  kable_styling()
...
no2_sensitivity %>%
  ...
  ggplot(., aes(y=exposure_model, #col=pollutant, 
                x=hr, xmin=lower95, xmax=upper95,)) + 
  geom_vline(xintercept = 1, alpha=0.5, linetype=2) + 
  geom_pointrange(position = position_dodge(width = 1)) +
  theme(legend.position = "none") +
  labs(x= "HR (95% CI)", y="NO2 Model", col = "Exposure Model", shape="")

ggsave(file.path(image_path, "presentation", "no2_sensitivity.png"), width = 5, height = 2.5)
...
sensitivity_lvs <- c("Main Analysis", 
                     "Extended Cohort (1994+)", "Restricted Cohort (2010+)",
                     "Restricted Cohort (Pre 2019)",
                     "Baseline Age Adjustment", "Calendar Time Axis & 2yr Age Adjustment", "No IPW")

sensitivity_hrs <- hrs %>%
  filter((grepl("Restricted|No IPW|Baseline Age Adjustment|Time Axis", description) | 
...
# plots 
ggplot(sensitivity_hrs, aes(y=description, col=pollutant, x=hr, xmin=lower95, xmax=upper95,)) + 
    facet_grid(cols=vars(pollutant), #cols = vars(outcome), 
               space="free", scales="free_y", #switch = "both"
               ) + 
    geom_vline(xintercept = 1, alpha=0.5, linetype=2) + 
    geom_pointrange(position = position_dodge(width = 1)) +
    #geom_text(aes(label=hr_label, x=Inf, hjust=1), position = position_dodge(width = 1), size=3)+
  theme(legend.position = "none") +
    labs(x= "HR (95% CI)", y="Sensitivity Analysis", col = "Exposure Model", shape="")

ggsave(file.path(image_path, "presentation", "other_sensitivity.png"), width = 6, height = 3)
...
# plot [same as above]
ggplot(secondary_hrs, aes(y=description, col=pollutant, x=hr, xmin=lower95, xmax=upper95)) + 
  facet_grid(cols=vars(pollutant), space="free", scales="free_y") + 
  geom_vline(xintercept = 1, alpha=0.5, linetype=2) + 
  geom_pointrange(position = position_dodge(width = 1)) +
  theme(legend.position = "none") +
  labs(x= "HR (95% CI)", y= "", #"Secondary Analysis", 
       col = "Outcome", shape="")
