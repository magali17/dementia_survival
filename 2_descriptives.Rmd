...
# --> could add inclusion/exclusion count diagram
...
# --> move more recent cohort? use other program to add what is dropped?

```{r, eval=F}
# --> ERROR: made me download libs causing issues now???!
library(DiagrammeR)
grViz("digraph flowchart {
      ...
      [1]: paste('Full Cohort (1994-2020)', '\\n n=____', '\\npy=___')
      [2]: paste('More Recent Cohort (2005-2020)', '\\n n=____', '\\npy=___')
      [3]: paste('APOE Status Available', '\\n n=____', '\\npy=___')
      [4]: paste('≥ 95% of Residential Histories Within MM Region')
      [5]: paste('10 Year Exposure', '\\n n=____', '\\npy=___')
      [6]: paste('5 Year Exposure', '\\n n=____', '\\npy=___')
      [7]: paste('1 Year Exposure', '\\n n=____', '\\npy=___')
      ")
```
...
# --> no dementia cases between 1995/6-1997?
...
## Completeness of Follow-Up Index (Clark 2002). 

Paul Crane updated this ~2019/2018 and it was 94.5%


It is the ratio of the total observed person-time of follow-up as a percentage of the potential time of follow-up in a study. We define a participant’s potential follow-up as the time from their entry into the study to the cut-off date for the study or the date of the event if it occurred.

The numerator of this expression is the sum of follow-up times, irrespective of whether the event took place or the survival time was censored.
  * follow-up years

The denominator is the sum of the **potential** follow-up times. 

? how to deal w/ ppl who never have 1 follow-up or those that just leave? We don't know what their **potential** follow-up time would have been since we don't know if they ever get dementia (we only know if they die at some point, but not if they died with or without dementia)

  * could predict whether they are likely to get dementia or not and see the avg age when someone gets dementia with their characteristics?

* ideally, we'd like to follow-up people until they stop being followed b/c: 
    * get dementia (not necessarily until they die?), the sur0$discontinue_reason is die (check that deathdt is close to last visit date) or have a terminal illness 
    * ??what is "Disc Referral" and "Disc Annual"?

?? if ppl die (and don't have "terminal illness"), their time should be deathdt-2 years? Assume that if they had not died, they would have attended their next follow-up..doest make sese b/c could have gotten dementia beforehand and that is when the denominaotr clock woul dstop?




```{r}
# > unique(sur$discontinue_reason)
# [1] ""                  "Refused"           "Other"             "Terminal Illness"  "Disc Referral"    
# [6] "Moved Out of Area" "Unable to Locate"  "Disc Annual"      


```

# --> ? could just summarize the # people who are lost to follow-up by reason. Don't count those that had a terminal illness or died. 

* hard to otherwise quantify potential FU time until someone Would have gotten dementia. 
* another paper idea? what have others done? see Weuve papers
